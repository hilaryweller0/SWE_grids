...
for it = 1:nt
    ...
    for i = 2:length(x)-1; for j = 1:length(y)-1;
        vu = 0.25*(v(i-1,j) + v(i,j) + v(i-1,j+1) + v(i,j+1));
        u(i,j) = u(i,j) + dt*(f*vu - (phi(i,j) - phi(i-1,j))/dx);
    endfor; endfor
    % Update y component of velocity
    for i = 1:length(x)-1; for j = 2:length(y)-1;
        uv = 0.25*(u(i,j-1) + u(i+1,j-1) + u(i,j) + u(i+1,j));
        v(i,j) = v(i,j) + dt*(-f*uv - (phi(i,j) - phi(i,j-1))/dy);
    endfor; endfor
    % Update phi
    for i = 1:length(x); for j = 1:length(y);
        phi(i,j) = phi(i,j) - dt*Phi*((u(i+1,j)-u(i,j))/dx + (v(i,j+1)-v(i,j))/dy);
    endfor; endfor

    plotCgrid(phi, u, v, it+1);
