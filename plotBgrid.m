...
% x and y positions of vertices (for velocity) and cell centres
x = 0:size(phi,1);
y = 0:size(phi,2);
xc = 0.5:size(phi,1);
yc = 0.5:size(phi,2);
...
quiver(y, x, v*vscale, u*vscale, 0, 'color', [0 0 0])
quiver(y, x, v*vscale, u*vscale, 0, 'color', [0 0 0])

title(sprintf('B-grid results for time step %g', it-1), 'fontsize', 16)
